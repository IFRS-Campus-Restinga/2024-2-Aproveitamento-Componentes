    ...
    return (
        <form className={styles.formContainer} onSubmit={handleSubmit}>
            <div className={styles.typeContainer}>
                ...
                <div>
                    {selectedNotice ? (
                        <span key={selectedNotice.id}> {selectedNotice.number} -{' '}
                            {new Date(selectedNotice.documentation_submission_start).toLocaleDateString()} a{' '}
                            {new Date(selectedNotice.documentation_submission_end).toLocaleDateString()} </span>
                    ) : (
            ...
            <div className={styles.formBtnContainer}>
                ...
                <Button onClick={() => (window.location.href = `/requests`)} type={"submit"}
                        style={{marginLeft: 'auto'}}>Enviar</Button>
            </div>
        </form>
    );
};
